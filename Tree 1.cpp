...
void LevelOrderTraversal(node*root){  //also called a Breadth first search where we print each levels
    queue<node*>q;
    q.push(root);
    while(!q.empty()){
        node*tmp=q.front();
...
void PreOrderTraversal(node*root){    //NLR or Root-Left-Right
    ...
    cout<<root->data<<" ";        //Call for Node
    PreOrderTraversal(root->left);//Call for Left
    PreOrderTraversal(root->right);//Call for Right
}
...
int main(){
    ...
    root=BuildTree(root);//This function is building the tree
